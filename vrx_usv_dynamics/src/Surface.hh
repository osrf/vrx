...
namespace vrx
{
  ...
  ///
  /// This plugin is a wave field CONSUMER. It reads the `Wavefield` component
  /// from the world entity - written by the world's wave source system, e.g.
  /// `gz-sim-waves-gerstner-system` - and accounts for the surface elevation
  /// at each point. No per-plugin wave configuration is needed or accepted: one
  /// wave field, many consumers. If the world has no wave source, every point
  /// sees still water and the plugin still floats the vessel.
  ///
  /// Prefer an analytic wave engine (Gerstner) over the FFT one for buoyancy:
  /// `FFTWaveSimulation::Elevation` ignores its time argument and samples the
  /// grid last built by the throttled `Advance`, whereas physics runs far
  /// faster - so buoyancy would sample a stale field.
  ///
  ...
  ///
  /// * `<hull_length>` is the length of the vessel [m].
  ...
  ///              by this plugin will be applied. Each `<point>` holds its
  ///              position relative to `link_name` as its own value.
  ///
  /// ## Example
  /// <plugin
  ///   filename="vrx-surface-system"
  ///   name="vrx::Surface">
