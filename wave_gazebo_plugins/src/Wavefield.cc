...
// WaveParameters
  ...
  void WaveParameters::FillMsg(gazebo::msgs::Param_V& _msg) const
  {
		    ...
		    // "model"
    {
      ...
      nextParam->set_name("model");
      nextParam->mutable_value()->set_type(gazebo::msgs::Any::STRING);
      nextParam->mutable_value()->set_string_value(this->data->model);
    }
    // "number"
    {
      auto nextParam = _msg.add_param();
    ...
    {
      auto nextParam = _msg.add_param();
      nextParam->set_name("gain");
      nextParam->mutable_value()->set_type(gazebo::msgs::Any::DOUBLE);
      nextParam->mutable_value()->set_double_value(this->data->gain);
    }
    // "direction"
    {
      ...
      nextParam->mutable_value()->mutable_vector3d_value()\
				->set_x(direction.X());
      nextParam->mutable_value()->mutable_vector3d_value()\
				->set_y(direction.Y());
  ...
  {
		this->data->model = Utilities::MsgParamString(_msg, "model", \
																									this->data->model);
    this->data->number = Utilities::MsgParamSizeT(_msg, "number", \
																									this->data->number);
    this->data->amplitude = Utilities::MsgParamDouble(_msg,   "amplitude", \
																											this->data->amplitude);
    this->data->period = Utilities::MsgParamDouble(_msg, "period", \
																									 this->data->period);
    this->data->phase = Utilities::MsgParamDouble(_msg, "phase", \
																									this->data->phase);
    this->data->direction = Utilities::MsgParamVector2(_msg, "direction", \
																											 this->data->direction);
    this->data->scale = Utilities::MsgParamDouble(_msg, "scale", \
																									this->data->scale);
    this->data->angle = Utilities::MsgParamDouble(_msg, "angle", \
																									this->data->angle);
    this->data->steepness = Utilities::MsgParamDouble(_msg, "steepness", \
																											this->data->steepness);
		this->data->tau = Utilities::MsgParamDouble(_msg, "tau", \
																								this->data->tau);
		this->data->gain = Utilities::MsgParamDouble(_msg, "gain",  \
																								 this->data->gain);
    this->data->Recalculate();
  }
