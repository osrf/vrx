...
class YawController(Node):
    def __init__(self):
        ...
        self.get_logger().info('yaw_controller initialized')


    def cmd_callback(self, msg : Twist) : 
        self.target_yaw = msg.linear.x
        self.get_logger().info('Recieved Command angle : "%s"' % self.target_yaw)

        self.lin_pid.set_setpoint(self.target_yaw)
    ...
    def send_output(self) :
        ...
        self.thruster_l_publisher.publish(msg)
        self.thruster_r_publisher.publish(msg)
        self.get_logger().info('\tPublishing: "%s" to rudder' % msg.data)
        # self.get_logger().info('\tPublishing: "%s" to left rudder' % msg.data)
        # self.get_logger().info('\tPublishing: "%s" to right rudder' % msg.data)
