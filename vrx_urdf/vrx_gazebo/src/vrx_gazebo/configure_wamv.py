...
class ConfigureWAMV(Node):

    def __init__(self):
        super().__init__('configure_wamv')

        self.declare_parameter('thruster_yaml', '')
        self.declare_parameter('component_yaml', '')
        self.declare_parameter('wamv_target', '')
        self.declare_parameter('wamv_gazebo', '')
        self.declare_parameter('wamv_locked', '')
        self.declare_parameter('components_dir', '')
        self.declare_parameter('thrusters_dir', '')

        # Check if yaml files were given
        thruster_yaml = self.get_parameter('thruster_yaml').get_parameter_value().string_value
        component_yaml = self.get_parameter('component_yaml').get_parameter_value().string_value
        ...
        if received_thruster_yaml:
            thruster_compliant = self.create_thruster_xacro()

        # Setup component xacro
        if received_component_yaml:
            component_compliant = self.create_component_xacro()

        # Setup command to generate WAM-V urdf file
        wamv_target = self.get_parameter('wamv_target').get_parameter_value().string_value
        wamv_gazebo = self.get_parameter('wamv_gazebo').get_parameter_value().string_value
        wamv_locked = self.get_parameter('wamv_locked').get_parameter_value().string_value
        ...
        if received_thruster_yaml:
            # thruster_yaml = self.get_parameter('thruster_yaml')
        ...
        if received_component_yaml:
            # component_yaml = self.get_parameter('component_yaml')
    ...
    def create_thruster_xacro(self):
        """
        ...
        thrusters_dir = self.get_parameter('thrusters_dir').get_parameter_value().string_value + '/'
    ...
    def create_component_xacro(self):
        """
        ...
        components_dir = self.get_parameter('components_dir').get_parameter_value().string_value + '/'
...
def main(args=None):
    ...
    configure_wamv  = ConfigureWAMV()
    rclpy.spin(configure_wamv)
    configure_wamv.destroy_node()
    rclpy.shutdown()
