  ...
  const geographic_msgs::GeoPoseStamped::ConstPtr &_msg)
{
  ...
  {
    ROS_WARN_NAMED("ObjectChecker",
      "Receiving multiple ID messages for same trial.  Ignoring.");
    return;
  }
...
namespace gazebo
{
  ...
  struct PopulationPluginPrivate
  {
    ...
    /// \brief Equivalent time since last object spawning, taking rate modifier
    /// into account.
    public: double elapsedEquivalentTime = 0.0;

    /// \brief Rate modifier of the populating: 1.0 will populate at the
    /// standard rate, other values will scale the populating frequency.
...
void PopulationPlugin::OnUpdate()
{
  ...
  auto elapsedTime =
    this->dataPtr->world->GetSimTime() - this->dataPtr->lastUpdateTime;

  // The rate modifier has the effect of slowing down sim time in this plugin.
  // If the rate modifier is 0.5, we will wait for twice as much time before
  // spawning a new object.
  this->dataPtr->elapsedEquivalentTime +=
    elapsedTime.Double() * this->dataPtr->rateModifier;
  if (this->dataPtr->elapsedEquivalentTime >=
      this->dataPtr->objects.front().time)
  {
    ...
    this->dataPtr->objectChecker->NewTrial(obj.name, obj.pose);
...
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  gzdbg << "PopulationPlugin: received activation request: " << _msg->data()
        << std::endl;
...
{
  std::lock_guard<std::mutex> lock(this->dataPtr->mutex);
  gzdbg << "PopulationPlugin: received rate modification request: "
        << _msg->data() << std::endl;
  ...
  {
    gzdbg << "Ignoring rate modification request with negative value: "
          << _msg->data() << std::endl;
  }
}
