...
class FFTWaveSimulation final : public IWaveField
{
  ...
  private: double tileSize_{200.0};
  /// \brief Grid resolution per axis (power of two).
  private: std::size_t gridSize_{128};
  /// \brief Wind speed [m/s], derived from `period`.
  private: double windSpeed_{0.0};
  /// \brief Spectrum amplitude gain.
  private: double gain_{1.0};
  ...
  private: Eigen::MatrixXd minEGrid_;
  ...
  private: mutable WaveField2D field_;
  ...
  private: std::unique_ptr<EncinoState> encino_;
  ...
  private: double encinoScale_{1.0};
  ...
  private: double lastUpdateT_{-1.0};
};
